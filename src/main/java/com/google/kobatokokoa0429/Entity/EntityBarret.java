package com.google.kobatokokoa0429.Entity;
 ...
 * 発射されるエンティティのクラス。
 * */
public class EntityBarret extends Entity implements IProjectile {
	...
	public EntityBarret(World par1World) {
		super(par1World);
	 ...
	 *            :弾の速度計算に使われる値2
	 * @param adjustX
	 *            :プレイヤーから見て水平方向に、発射する弾をずらす(複数発射するときなどに使用する)
	 * @param adjustZ
	 *            :プレイヤーから見て前後方向に弾をずらす
	 * @param adjustY
	 *            :プレイヤーから見て上下方向に弾をずらす
	 */
	public EntityBarret(World par1World, EntityLivingBase par2EntityLivingBase, float speed, float speed2,
		...
		this.setThrowableHeading(this.motionX, this.motionY, this.motionZ, speed * 6F, speed2);
	}

	/* dataWatcherを利用したサーバ・クライアント間の同期処理だと思う */
	/** 恐らくInitという名前的に初期化処理だと思われます。。 */
	protected void entityInit() {
		this.dataWatcher.addObject(16, Byte.valueOf((byte) 0));
	}

	/*
	 * IProjectileで実装が必要なメソッド。 ディスペンサーによる発射メソッドなどで使用されている。
	 */
	...
	/*
	 * Tick毎に呼ばれる更新処理。 速度の更新、衝突判定などをここで行う。
	 */
	public void onUpdate() {
		...
		if (this.inGround) {
			...
			if (j == this.inTile && k == this.inData) {
				...
				int limit = this.isPenetrateBlock() ? 2 : 2;
		...
		} else// 埋まってない時。速度の更新。
		{
			...
			/*
			 * 当たったエンティティそれそれについての判定部分。 ここで特定の種類のエンティティに当たらないようにできる。
			 */
			...
			if (movingobjectposition != null)// blockのみ
			{
				if (!this.isPenetrateBlock()) // エンティティには当たってない。ブロックに当たった。
				{
					...
					if (!worldObj.isRemote) {
						...
						// posZ, 100F, true);
						// worldObj.spawnEntityInWorld(new
						// EntityLightningBolt(worldObj, posX, posY, posZ));
			...
			if (this.isPenetrateBlock()) {
				for (int n = 0; n < 4; ++n) {
					this.worldObj.spawnParticle("crit", this.posX + this.motionX * (double) n / 4.0D,
							this.posY + this.motionY * (double) n / 4.0D, this.posZ + this.motionZ * (double) n / 4.0D,
	 ...
	 */
	/**
	 * 変更・引数の変数名 変更・NBTTagCompoundのバッファを一枚挟む
	 */
	public void writeEntityToNBT(NBTTagCompound tagCompound) {
		...
		compound.setByte("inTile", (byte) (byte) Block.getIdFromBlock(this.inTile));
		compound.setByte("inData", (byte) this.inData);
		compound.setByte("inGround", (byte) (this.inGround ? 1 : 0));
		compound.setDouble("damage", this.damage);
		tagCompound.setTag("BarretInfo", compound);
	}
	...
	public void readEntityFromNBT(NBTTagCompound tagCompound) {
		NBTTagCompound compound = tagCompound.getCompoundTag("BarretInfo");
	...
	/*
	 * ブロックに対し、上を歩いたかという判定の対象になるか、というEntityクラスのメソッド。 耕地を荒らしたりするのに使う。
	 */
